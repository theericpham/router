 ...
 */
FILE *
dump_open(const char *fname, int thiszone, int snaplen)
{       
  FILE *fp;
        ...
        if (fname[0] == '-' && fname[1] == '\0')
                ...
                fp = fopen(fname, "w");
                if (fp == NULL) {
                        fprintf(stderr, "dump_open: can't open %s",
                            fname);
                        return (NULL);
                }
        }
 ...
 */
void
dump(FILE *fp, const struct pcap_pkthdr *h, const unsigned char *sp)
{
